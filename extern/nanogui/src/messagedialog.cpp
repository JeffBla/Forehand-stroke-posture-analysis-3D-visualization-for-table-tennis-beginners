...
NAMESPACE_BEGIN(nanogui)
    ...
    MessageDialog::MessageDialog(Widget *parent, Type type, const std::string &title, const std::string &message,
                                 int message_font_size, const std::string &button_text,
                                 const std::string &alt_button_text, bool alt_button)
            : Window(parent, title) {
        ...
        switch (type) {
            case Type::Information:
                icon = m_theme->m_message_information_icon;
                break;
            case Type::Question:
                icon = m_theme->m_message_question_icon;
                break;
            case Type::Warning:
                icon = m_theme->m_message_warning_icon;
                break;
        }
        Label *icon_label = new Label(panel1, std::string(utf8(icon).data()), "icons");
        icon_label->set_font_size(50);
        m_message_label = new Label(panel1, message, "sans-bold", message_font_size);
